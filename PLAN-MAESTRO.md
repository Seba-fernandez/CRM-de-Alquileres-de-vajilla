...
### **FASE 3 — Aviso de pedido nuevo** ✅ *(completada y confirmada en vivo — 02/09/2026)*
...
- [x] Trigger de Postgres (`pg_net`) en `orders` INSERT `canal='web'` → llama a
      ...
      repo (es público).
- [x] Sebastián activó CallMeBot y cargó los 3 secrets (`CALLMEBOT_PHONE`,
      `CALLMEBOT_APIKEY`, `WEBHOOK_SECRET`). En el camino apareció un typo en
      `CALLMEBOT_PHONE` (código de área 351 de Córdoba tipeado como 451) — se
      diagnosticó comparando las respuestas de CallMeBot entre pruebas y se
      corrigió del lado del dashboard.
- [x] **Confirmado con un pedido de prueba real (creado y borrado):** CallMeBot
      respondió `200 · "Message queued"` — el WhatsApp llega. Fase cerrada.
...
**Resultado:** cada pedido nuevo de la web te llega por WhatsApp al toque, sin
depender de que el cliente mande el suyo.
